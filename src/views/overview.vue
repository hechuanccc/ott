    ...
    <div class="box m-b-lg">
        ...
        <div class="table-box" v-show="active === 'table'">
            <table class="table table-striped b-t">
                ...
                <tbody>
                    <tr v-for="d in daily | orderBy 'date' -1">
    ...
    <div class="box">
        <table class="table table-striped b-t">
            <thead>
                <tr>
                    <th>&nbsp;</th>
                    <th v-if="$root.$data.userType !== 'agent'">代理</th>
                    <th>新会员数</th>
                    <th>存款新会员数</th>
                    <th>存款总额</th>
                    <th>取款总额</th>
            ...
            <tbody v-if="$root.$data.userType !== 'agent'">
                <tr v-for="d in periodic" >
                    <td>{{$t('common.' + d.report_age)}}</td>
                    <td>{{d.active_agent}}</td>
                    <td><a v-link="'/member?created_at_0=' + dateSet[d.report_age][0] + '=&created_at_1=' + dateSet[d.report_age][1] + '&report_flag=True'">{{d.new_member_count}}</a></td>
                    <td>{{d.deposit_member_count}}</td>
                    <td><a v-link="'/bill/search?transaction_type[]=remit&transaction_type[]=online_pay&created_at_0=' + dateSet[d.report_age][0] + '=&created_at_1=' + dateSet[d.report_age][1] + '&report_flag=True'">{{d.deposit_amount | currency '￥'}}</a></td>
                    <td><a v-link="'/bill/search?transaction_type[]=withdraw&created_at_0=' + dateSet[d.report_age][0] + '=&created_at_1=' + dateSet[d.report_age][1] + '&report_flag=True'">{{d.withdraw_amount | currency '￥'}}</a></td>
                    <td><a v-link="'/report/game?date_0=' + dateSet[d.report_age][0] + '=&date_1=' + dateSet[d.report_age][1] + '&dateRange=' + d.report_age">{{d.valid_bet | currency '￥'}}</a></td>
                    <td><a v-link="'/report/game?date_0=' + dateSet[d.report_age][0] + '=&date_1=' + dateSet[d.report_age][1] + '&dateRange=' + d.report_age">{{d.profit | currency '￥'}}</a></td>
                </tr>
