        ...
        <div class="navbar">
            ...
            <div class="dropdown pull-right my">
                <button class="btn white btn-sm dropdown-toggle" @click="dropdown=!dropdown">
                    <span class="dropdown-label">{{$root.username}}111</span>
                            ...
                            <div class="search-results" v-if="hasResults">
                                <div class="search-items">
                                    <div class="search-item" v-for="r in results">
                                      <router-link class="nav-link" to="'/member/' + r.id">
                                        <a>{{r.username}}</a>
                                      </router-link>
                                    </div>
                                </div>
            ...
            <div class="navbar-nav pull-right" v-if="$root.$data.userType !== 'agent'">
              <router-link class="nav-link" to="'/member?logined=1'">
                <a><span class="m-r label success" >{{$t('common.onlinemembers')}}: {{members_count}} </span></a>
              </router-link>
            </div>
        </div>
    </div>
